...
export class Con extends Canvas {
  ...
  constructor(opt: any) {
    ...
    for(let i = 0; i < Conf.instance.ITEM_NUM; i++) {
      if(i % this._line == 0) this._makeColors()
  ...
  protected _update(): void {
    ...
    const block = Math.min(w, h) / this._line
    this._item.forEach((val) => {
      val.position.x = val.itemPos.y * block - Math.min(w, h) * 0.5 + block * 0.5
      val.position.y = val.itemPos.x * -block + this._maxY * block * 0.5 + block * 0.5
      val.position.z = block * val.itemPos.x - this._maxY * block * 0.5 + block * 0.5
    })
